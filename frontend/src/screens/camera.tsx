...
const CameraScreen = () => {
  ...
  const registerGotcha = () => {
    ...
    axios
      .post(`/attempt/${user._id}`, formData, {
        headers: {
  ...
  return (
    <div className="flex flex-col justify-start items-center h-screenNav w-full">
      <div className="container border-r border-l py-4 border-gray-300 flex flex-col justify-between flex-1">
        ...
        {!state?.tempImage && (
            <OpenCamera>
              <div className="w-full h-full border-2 border-dashed border-gray-400 flex flex-col items-center justify-center cursor-pointer rounded-xl">
                ...
                <p className="text-gray-400 font-roboto text-lg font-bold">
                  Click to add image
                </p>
              </div>
            </OpenCamera>
        ...
        <TitleAndText title="Question:" text={user?.target?.question} />
        ...
        <Button className="my-4" onClick={() => registerGotcha()}>Gotcha</Button>
      </div>
    </div>
  );
};
